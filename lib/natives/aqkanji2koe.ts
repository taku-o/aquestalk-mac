...
class AqKanji2KoeLib {
  ...
  constructor(frameworkPath: string) {
    ...
    const frameworkExePath = path.join(frameworkPath, 'Versions/A/AqKanji2Koe');
    const ptr_AqKanji2Koe_Create = ffi.DynamicLibrary(frameworkExePath).get('AqKanji2Koe_Create');
    const ptr_AqKanji2Koe_Release = ffi.DynamicLibrary(frameworkExePath).get('AqKanji2Koe_Release');
    const ptr_AqKanji2Koe_Convert = ffi.DynamicLibrary(frameworkExePath).get('AqKanji2Koe_Convert');
    const ptr_AqKanji2Koe_SetDevKey = ffi.DynamicLibrary(frameworkExePath).get('AqKanji2Koe_SetDevKey');
